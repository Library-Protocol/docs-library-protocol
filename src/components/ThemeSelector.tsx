    ...
    <Listbox as="div" value={theme} onChange={setTheme} {...props}>
      ...
      >
        <LightIcon
          className={clsx(
            'h-4 w-4 dark:hidden',
            theme === 'system' ? 'fill-slate-400' : 'fill-amber-900',
          )}
        />
        <DarkIcon
          className={clsx(
        ...
        {themes.map((theme) => (
          <ListboxOption
            ...
            className={({ focus, selected }) =>
              clsx(
                'flex cursor-pointer items-center rounded-[0.625rem] px-3 py-1 text-sm text-slate-700 dark:text-slate-400',
                {
                  'text-amber-900 dark:text-amber-500': selected,
                },
              )
            }
          >
                ...
                <div className="rounded-md bg-white p-1 shadow ring-1 ring-slate-900/5 dark:bg-slate-700 dark:ring-inset dark:ring-white/5">
                  <theme.icon
                    className={clsx(
                      'h-4 w-4',
                      selected
                        ? 'fill-amber-900 dark:fill-amber-600'
                        : 'fill-slate-400',
                    )}
                  />
                </div>
